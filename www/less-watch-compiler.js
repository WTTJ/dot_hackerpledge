...
module.exports = function(less_bin, argvs) {
  ...
  function compileCSS(file){
      ...
      var command = less_bin + ' -x '+file.replace(/\s+/g,'\\ ')+' '+argvs[1]+'/'+filename.replace(/\s+/g,'\\ ')+'.css';
      // Run the command
  ...
  console.log('Watching directory '+ argvs +' for file changes.');
  console.log('');
  watchTree(
    argvs[0],
